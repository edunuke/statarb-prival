# ==============================================================================
# STAT-ARB PARAMETER TUNING ENGINE
# Runs statarb_2.5.py pipeline with systematic parameter variations
...
spec = importlib.util.spec_from_file_location(
    "statarb_pipeline",
    os.path.join(os.path.dirname(__file__), "statarb_2.5.py")
)
...
# Search space: most impactful parameters first
PRIMARY_KEYS = ["entry_z_spread", "exit_z_spread", "max_hurst_exponent"]
SECONDARY_KEYS = [
    "max_slope_tstat", "promote_top_n", "ghost_truncation_quantile",
    "max_gross_exposure", "stop_loss_z", "p_value_threshold", "min_correlation"
]

PARAM_GRID = {
    # Core entry/exit thresholds
    "entry_z_spread": [0.75, 0.85, 0.95, 1.05, 1.15],
    "exit_z_spread":  [0.20, 0.25, 0.30, 0.35, 0.40],
    "max_hurst_exponent": [0.38, 0.40, 0.43, 0.45, 0.48],
    
    # Secondary filters
    "max_slope_tstat": [1.50, 1.75, 2.00, 2.25],
    "promote_top_n": [30, 40, 50, 60, 75],
    "ghost_truncation_quantile": [0.10, 0.15, 0.20, 0.25, 0.30],
    
    # Risk bounds
    "max_gross_exposure": [1.20, 1.50, 1.80, 2.00],
    "stop_loss_z": [2.50, 2.80, 3.00, 3.50],
    
    # Statistical filters
    "p_value_threshold": [0.15, 0.20, 0.25],
    "min_correlation": [0.35, 0.40, 0.45],
}

RESULTS_FILE = "artifacts/tuning_results.json"
...
def run_single_strategy(params, run_label):
    ...
    safe_label = run_label.replace(" ", "_").replace("/", "_").replace(",", "")
    params["state_persistence_file"] = f"artifacts/tuning_runs/state_{safe_label}_{ts}.pkl"
    params["model_id"] = f"tune_{safe_label}_{ts}"
...
def main():
    print("=" * 85)
    print("STAT-ARB PARAMETER TUNING ENGINE STARTED")
